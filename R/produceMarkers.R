produceMarkers <- function(seurat.obj,
                              cells.per.ident = Inf,
                              top.gene.plot = TRUE){ 
  
  date <- gsub("-","",Sys.Date())
  markers <- FindAllMarkers(object = seurat.obj, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25, max.cells.per.ident = cells.per.ident)
  markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
  write.table(markers %>% group_by(cluster), file= paste0(date,"markers.txt"), row.names=FALSE, sep="\t")
  if(top.gene.plot == TRUE){
    cluster.averages <- AverageExpression(object = seurat.obj, return.seurat = TRUE)
    top3 <- markers %>% group_by(cluster) %>% top_n(n = 3, wt = avg_logFC)
    print(pheatmap(GetAssayData(cluster.averages)[top3$gene,]))
    }
  }
